...
public class Point_2D {
	...
	public void scale(Point_2D cen, double ratio) {
        /**we prefer to do it this way "just in case" , we want only the two option to work
         * even if someone changed the code in Ex4.
         * its not necessary to do it this way*/
        if(ratio == 110) {
            double x = cen.x();
            double y = cen.y();
            this._x = x + (this._x - x)*1.1;
            this._y = y + (this._y - y)*1.1;
        }
        if(ratio == 90) {
            double x = cen.x();
            double y = cen.y();
            this._x = x + (this._x - x)*0.9;
            this._y = y + (this._y - y)*0.9;
        }
	}
